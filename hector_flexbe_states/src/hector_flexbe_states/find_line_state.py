...
class FindLineState(EventState):
	...
	def __init__(self, distance=1, velocity_topic='/cmd_vel'):
		'''
		Constructor
		'''
		super(FindLineState, self).__init__(outcomes=['reached'],input_keys=['reverse', 'speed'])
	...
	def execute(self, userdata):
		...
		duration = self._distance / userdata.speed;
	...
	def on_enter(self, userdata):
		self._reached = False
		self._start_time = rospy.Time.now()
		self._msg.linear.x = userdata.speed
		if (userdata.reverse):
